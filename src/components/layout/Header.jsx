...
const HeaderNav = memo(({ navbar, mount }) => {
  return (
    <motion.nav key={navbar} initial={mount ? { ...(navbar ? { color: "rgb(255, 255, 255)", backdropFilter: "blur(2.5px)", background: "rgba(255,255,255,0)" } : { backgroundColor: "rgb(255, 255, 255)", color: "rgb(34, 34, 34)", backdropFilter: "none" }) } : false} animate={{ ...(navbar ? { backgroundColor: "rgb(255, 255, 255)", color: "rgb(34, 34, 34)", backdropFilter: "none" } : { color: "rgb(255, 255, 255)", backdropFilter: "blur(2.5px)", background: "rgba(255,255,255,0)" }) }} transition={{ type: "tween", duration: 0.5, ease: "linear" }} id="logoIcon">
      <h1 id="logo">
...
const Header = () => {
  ...
  return (
    <AnimatePresence>
      {!header && (
        <motion.header initial={hasMounted.current ? { y: -150 } : false} animate={{ y: 0 }} exit={{ y: -150 }} transition={{ type: "tween", duration: 0.5, ease: "linear", when: "beforeChildren" }} id="header">
          <div id="curLangHead">
