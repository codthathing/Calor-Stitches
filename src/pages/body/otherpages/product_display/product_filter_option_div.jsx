...
const ProductFilterOptionDiv = ({ filterText, productFunction, productArray, setProductArray, textLengthId, showColorDiv, showFirstInput, showSecondInput, showTypeLength, showSizeLength, showColorLength }) => {
    ...
    <div className="productFilterMainDivs">
      ...
      <section className="productFilterSections">
        {productArray.map(({ id, text, color, option, style }) => {
          return (
            <div key={id} 
            onMouseEnter={() => ChangeTextStyleHover(id, true)} onMouseLeave={() => ChangeTextStyleHover(id, false)} 
            onClick={() => productFunction(id, option, text, productArray, setProductArray)} className="productFilterColorDiv">
...
export default ProductFilterOptionDiv;