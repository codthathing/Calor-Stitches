...
const FooterLinkTemplate = () => {
  const linkMain = [
    { id: 0, linkHead: "Shop", type: "link", linkTexts: [{ id: 0, text: "dresses" }, { id: 1, text: "t-shirts" }, { id: 2, text: "jeans" }, { id: 3, text: "outerwear" }, { id: 4, text: "activewear" }, { id: 5, text: "sweaters" }] },
          ...
          <main key={id} className="linkMain">
            ...
            <nav className="linkNav">
              {linkTexts.map(({ id, text }) => {
                return (
                  <Fragment key={id}>
                    {(type === "link") ? <Link to={`/product/collection/${text}`} className="linkText">{text}</Link> : <p key={id} className="linkText">{text}</p>}
                  </Fragment>
                );
              })}
            </nav>
