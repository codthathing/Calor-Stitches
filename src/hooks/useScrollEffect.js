...
export const PageScrollToTop = memo(({ showIcon }) => {
  ...
  return (
    <AnimatePresence>
      {showIcon && (
        <motion.div id="page-top-div" initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }} transition={{ type: "tween", duration: 0.5, ease: "linear" }} onClick={() => scrollToSection(home_section)}>
