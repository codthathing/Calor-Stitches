    ...
    <AnimatePresence onExitComplete={() => setToggleSideMenu(false)}>
      {!header && (
        <motion.header initial={hasMounted.current && { y: -150 }} animate={{ y: 0 }} exit={{ y: -150 }} transition={{ type: "tween", duration: 0.35, ease: "linear", when: "afterChildren" }} id="header">
          {children}
          <motion.nav initial={(hasMounted.current && !navbar) && { ...(navbar ? { color: "rgb(255, 255, 255)", backdropFilter: "blur(2.5px)", background: "rgba(255,255,255,0)" } : { backgroundColor: "rgb(255, 255, 255)", color: "rgb(34, 34, 34)", backdropFilter: "none" }) }} animate={{ ...(navbar ? { backgroundColor: "rgb(255, 255, 255)", color: "rgb(34, 34, 34)", backdropFilter: "none" } : { color: "rgb(255, 255, 255)", backdropFilter: "blur(2.5px)", background: "rgba(255,255,255,0)" }) }} transition={{ type: "tween", duration: 0.35, ease: "linear" }} id="logoIcon">
            <h1 id="logo">
